    ...
    Vue.component("visitor-conversation-interaction", {
        ...
        template: `
            <section v-bind:id="id" class="reply-container">
                <emoji-menu 
                    v-if="ShowingEmojiMenu"
                    v-on:Clicked="EmojiClicked"
                />

                <file-menu 
                    :show="ShowingFiles"
                    :id="FileId"
                    :files="$store.state.uploadedFiles"
                    :domain="$store.state.currentChat.Domain"
                    v-on:Send="SendFile"
                />
                ...
                <div class="column is-full" style="padding-top:0.5rem;">
                    <div 
                        id="inputArea" 
                        v-bind:class="{'beingMonitored':monitoring}"
                        class="textarea" 
                ...
                <div class="column is-full" style="padding-top:0px;">
                    <div class="is-pulled-right chat-icons" style="position:relative">
                        <button 
                            v-if="site != null && site.AllowEmoji && $store.state.settings.ShowEmoji"
                            id="emojiBtn" 
                            class="emojiBtn emoji-icon has-tooltip-left"
                            data-tooltip="Emoji's"
                            v-bind:class="{'is-info':ShowingEmojiMenu}"
                            v-on:click="EmojiBtnClicked"
                            data-show="quickview"
                            data-target="responsesView"
                            v-bind:disabled="disabled"
                        >
                        ...
                        <button 
                            class="emojiBtn emoji-icon has-tooltip-left"
                            data-tooltip="Canned Responses"
                            v-bind:class="{'is-info':ShowingCannedResponses}"
                            v-on:click="$emit('CannedReponsesClicked')"
                            data-show="quickview"
                            data-target="responsesView"
                            v-bind:disabled="disabled"
                        >
                        ...
                        <button 
                            v-if="monitoring == false"
                            class="emojiBtn emoji-icon has-tooltip-left"
                            data-tooltip="Send File"
                            v-bind:class="{'is-info':ShowingFiles}"
                            v-on:click="SendFileClicked"
                            data-show="quickview"
                            data-target="responsesView"
                            v-bind:disabled="disabled"
                        >
                        ...
                        <button 
                            v-if="monitoring == false"
                            class="emojiBtn emoji-icon has-tooltip-left"
                            data-tooltip="Request File From Visitor"
                            v-on:click="RequestFileClicked"
                            data-show="quickview"
                            data-target="responsesView"
                            v-bind:disabled="disabled"
                        >
        ...
        beforeCreate() {
           ...
           hooks.register(events.Connection.MonitoredChat, (e) => {
            ...
            hooks.register(events.Chat.SuggestionFromServer, (suggestion) => {
                ...
                if(this.HasAttachments(suggestion)) {
                    this.InsertAttachments(suggestion);
                    return;
                } 
            ...
            hooks.register(events.Connection.ChatChanged, (e) => {
                ...
                ) 
                {
                    ...
                    if(hasPreviousChatInput) {
                        ...
                        if (this.HasAttachments(hasPreviousChatInput)) {
                            this.InsertAttachments(hasPreviousChatInput);
                            return;
                        }
                        ...
                        if
                        (
                            (this.$store.state.currentChatMessages.length <= 0 || opMessage == null) &&
                            this.InputArea().innerText == ""
                        ) 
                        {
                            this.InputArea().innerText = this.GreetingMessage();
                        }
                    }
        ...
        methods: { 
            ...
            HasAttachments(msg) {
                var attachmentStartTag = "<Attachment>";
                return msg.indexOf(attachmentStartTag) != -1;
            },
            ...
            SendFile(fileName, url) {
                ...
                var msg = {
                    code: 1,
                    msg: `<link><name>${fileName}</name><url>${url}</url></link>`,
                    date: getDate(new Date()),
                    isLink: true
                };

                if (state.chatMessages[state.currentChat.ChatUID] == null) {
                    state.chatMessages[state.currentChat.ChatUID] = [];
                }
